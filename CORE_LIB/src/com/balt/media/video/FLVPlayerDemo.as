/**
* FLVPlayer : as3 flv vplayer
*@author:   adriaan scholvinck 
*/
// FIXME - needs to be generalized to not rely on "Nokia Standard Light" font.
package com.balt.media.video {
	...
	public class FLVPlayerDemo extends FLVPlayer {
		...
		public function FLVPlayerDemo( $parent : MovieClip, $path : String = "", $videoPath : String = ""){
			...
			trace ( "AS3 DEMO FLV PLAYER VERSION 1.0" );
		}
		
		protected function xmlComplete() : void {
//			loadFont();
...
//			currentindex = super.getCuePointIndex(cuepoints, true, ns.time);
		...
		private function setText ( tf:TextField, str:String, size : Number = 20 ) : void {
			var font:Font = getAvailableFont ( new Array("Nokia Standard Light") , false, str );
			var format:TextFormat = new TextFormat();
			if ( font != null ) {
				format.font = "Nokia Standard Light";
				tf.embedFonts = true;
		   	} else {
				format.font = "_sans";
			   	tf.embedFonts = false;
